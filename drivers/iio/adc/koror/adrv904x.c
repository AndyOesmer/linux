...
// 		       adrv904x_phy_show, adrv904x_phy_store,
// 		       ADRV904X_INIT_CAL |
// 			       (ADI_ADRV904X_TX_LO_LEAKAGE_INTERNAL << 8));

// static IIO_DEVICE_ATTR(calibrate_tx_lol_ext_en, 0644,
// 		       adrv904x_phy_show, adrv904x_phy_store,
// 		       ADRV904X_INIT_CAL |
// 			       (ADI_ADRV904X_TX_LO_LEAKAGE_EXTERNAL << 8));
				     ...
				     const char *buf, size_t len)
{
	...
	case ADI_ADRV904X_LO0:
		...
		loConfig.loName = ADI_ADRV904X_LO0;
...
static const struct iio_chan_spec_ext_info adrv904x_phy_ext_lo_info[] = {
	 ...
	 */
	_ADRV904X_EXT_LO_INFO("frequency", ADI_ADRV904X_LO0),
	{},
};

static const struct iio_chan_spec_ext_info adrv904x_phy_ext_auxlo_info[] = {
	/* Ideally we use IIO_CHAN_INFO_FREQUENCY, but there are
	 * values > 2^32 in order to support the entire frequency range
	 * in Hz. Using scale is a bit ugly.
	 */
	_ADRV904X_EXT_LO_INFO("frequency", ADI_ADRV904X_LO1),
	{},
};

// static int adrv904x_set_agc_mode(struct iio_dev *indio_dev,
// 				 const struct iio_chan_spec *chan, u32 mode)
// {
// 	struct adrv904x_rf_phy *phy = iio_priv(indio_dev);
// 	adi_adrv904x_RxAgcMode_t gainMode;
// 	int ret;

// 	switch (mode) {
// 	case 0:
// 		gainMode.agcMode = ADI_ADRV904X_MGC;
// 		break;
// 	case 1:
// 		gainMode.agcMode = ADI_ADRV904X_AGCFAST;
// 		break;
// 	case 2:
// 		gainMode.agcMode = ADI_ADRV904X_AGCSLOW;
// 		break;
// 	case 3:
// 		gainMode.agcMode = ADI_ADRV904X_HYBRID;
// 		break;
// 	default:
// 		return -EINVAL;
// 	}

// 	gainMode.rxChannelMask = ADI_ADRV904X_RX1 << chan->channel;

// 	ret = adi_adrv904x_RxGainCtrlModeSet(phy->kororDevice, &gainMode, 1);
// 	if (ret)
...
// static int adrv904x_get_agc_mode(struct iio_dev *indio_dev,
// 				 const struct iio_chan_spec *chan)
// {
// 	struct adrv904x_rf_phy *phy = iio_priv(indio_dev);
// 	adi_adrv904x_AgcCfg_t agcConfigReadBack = { 0 };
// 	int ret;

// 	ret = adi_adrv904x_AgcCfgGet(
// 		phy->kororDevice, ADI_ADRV904X_RXOFF << chan->channel, &agcConfigReadBack);
// 	if (ret)
// 		return adrv904x_dev_err(phy);
...
// static const char *const adrv904x_agc_modes[] = { "manual", "fast_attack",
// 						  "slow_loop", "hybrid" };

// static const struct iio_enum adrv904x_agc_modes_available = {
// 	.items = adrv904x_agc_modes,
// 	.num_items = ARRAY_SIZE(adrv904x_agc_modes),
// 	.get = adrv904x_get_agc_mode,
// 	.set = adrv904x_set_agc_mode,
// };
				     ...
				     const char *buf, size_t len)
{
	...
	case RSSI:
		break;
	// case RX_QEC:
	// 	mask = ADI_ADRV904X_TRACK_RX1_QEC << chan->channel;

	// 	ret = adi_adrv904x_TrackingCalsEnableSet(
	// 		phy->kororDevice, mask,
	...
	// 	break;
	// case RX_HD2:
	// 	mask = ADI_ADRV904X_TRACK_RX1_HD2 << chan->channel;
	...
	// 	break;
	// case RX_DIG_DC:
	// 	ret = adi_adrv904x_DigDcOffsetEnableGet(phy->kororDevice, &mask16);
	// 	if (ret) {
	// 		ret = adrv904x_dev_err(phy);
	// 		goto out;
	// 	}
	...
	// 	if (enable)
	// 		mask16 |= (ADI_ADRV904X_MSHIFT_DC_OFFSET_RX_CH0 << chan->channel);
	// 	else
	// 		mask16 &= ~(ADI_ADRV904X_MSHIFT_DC_OFFSET_RX_CH0 << chan->channel);

	// 	ret = adi_adrv904x_DigDcOffsetEnableSet(phy->kororDevice, mask16);
	// 	if (ret)
	// 		ret = adrv904x_dev_err(phy);
	// 	break;
	default:
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	switch (private) {
	// case RSSI:
	// 	rxchan = ADI_ADRV904X_RX1 << chan->channel;

	// 	ret = adi_adrv904x_RxDecPowerGet(phy->kororDevice, rxchan,
	// 					 &dec_pwr_mdb);
	// 	if (ret == 0)
	...
	// 	break;
	// case RX_QEC:
	// 	tmask = ADI_ADRV904X_TRACK_RX1_QEC << chan->channel;
	// 	ret = adi_adrv904x_TrackingCalsEnableGet(phy->kororDevice, &mask);
	// 	if (ret == 0)
	// 		ret = sprintf(buf, "%d\n", !!(tmask & mask));

	// 	break;
	// case RX_HD2:
	// 	tmask = ADI_ADRV904X_TRACK_RX1_HD2 << chan->channel;
	// 	ret = adi_adrv904x_TrackingCalsEnableGet(phy->kororDevice, &mask);
	// 	if (ret == 0)
	// 		ret = sprintf(buf, "%d\n", !!(tmask & mask));
	...
	// 	if (ret == 0)
	// 		ret = sprintf(buf, "%d\n",
	// 			!!((ADI_ADRV904X_MSHIFT_DC_OFFSET_RX_CH0 << chan->channel) & mask16));
	// 	else
	// 		ret = adrv904x_dev_err(phy);
	// 	break;
				    ...
				    const struct iio_chan_spec *chan, char *buf)
{
	...
	switch (private) {
	// case TX_QEC:
	// 	tmask = ADI_ADRV904X_TRACK_TX1_QEC << chan->channel;
	// 	ret = adi_adrv904x_TrackingCalsEnableGet(phy->kororDevice, &mask);
	// 	val = !!(tmask & mask);
	// 	break;
	// case TX_LOL:
	// 	tmask = ADI_ADRV904X_TRACK_TX1_LOL << chan->channel;
	// 	ret = adi_adrv904x_TrackingCalsEnableGet(phy->kororDevice, &mask);
	// 	val = !!(tmask & mask);
	// 	break;
				     ...
				     const char *buf, size_t len)
{
	...
	switch (private) {
	// case TX_QEC:
	// 	mask = ADI_ADRV904X_TRACK_TX1_QEC << chan->channel;
	...
	// 	break;
	// case TX_LOL:
	// 	mask = ADI_ADRV904X_TRACK_TX1_LOL << chan->channel;
...
static const struct iio_chan_spec_ext_info adrv904x_phy_rx_ext_info[] = {
	...
	// 			  &adrv904x_agc_modes_available),
	// IIO_ENUM("gain_control_mode", false, &adrv904x_agc_modes_available),
	_ADRV904X_EXT_RX_INFO("rssi", RSSI),
	_ADRV904X_EXT_RX_INFO("quadrature_tracking_en", RX_QEC),
	_ADRV904X_EXT_RX_INFO("hd2_tracking_en",
				 ...
				 int *val2, long m)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		...
		if (chan->output)
			*val = !!(txchan & (ADI_ADRV904X_TX1 << chan->channel));
		else
			*val = !!(rxchan & (ADI_ADRV904X_RX1 << chan->channel));
				  ...
				  int val2, long mask)
{
	...
	case IIO_CHAN_INFO_ENABLE:
		...
		//ToDo ORx, clarify what is en and what is msk
		if (chan->output) {
			if (val)
				txchan_msk |= (ADI_ADRV904X_TX1 << chan->channel);
			else
				txchan_msk &= ~(ADI_ADRV904X_TX1 << chan->channel);
			txchan_en = txchan_msk;
		} else {
			if (val)
				rxchan_msk |= (ADI_ADRV904X_RX1 << chan->channel);
			else
				rxchan_msk &= ~(ADI_ADRV904X_RX1 << chan->channel);
			rxchan_en = rxchan_msk;
		}
...
static const struct iio_chan_spec adrv904x_phy_chan[] = {
	...
	},
	{
		...
		.channel = 2,
		.extend_name = "AUX_LO",
		.ext_info = adrv904x_phy_ext_auxlo_info,
	},
	{
		/* TX1 */
		.type = IIO_VOLTAGE,
				      ...
				      loff_t *ppos)
{
	...
	// adi_adrv904x_TxTestToneCfg_t toneCfg;
	u32 val2, val3, val4;
	s64 val;
	char buf[80];
	int ret;
	...
	// 	mutex_lock(&phy->lock);
	// 	ret = adi_adrv904x_SpiFieldWrite(phy->kororDevice, 0x6689,
	// 					 val ? 0x7 : 0, 0xE0, 5);
	// 	mutex_unlock(&phy->lock);
	// 	if (ret)
	// 		return ret;
	...
	// 	toneCfg.enable = val;
	// 	toneCfg.txChannelMask = ADI_ADRV904X_TXALL;
	// 	toneCfg.txToneFreq_Hz = val2;
	// 	if (ret == 3)
	// 		toneCfg.txToneGain = ADI_ADRV904X_TX_NCO_0_DB & val3;
	// 	else
	// 		toneCfg.txToneGain = ADI_ADRV904X_TX_NCO_0_DB;

	// 	mutex_lock(&phy->lock);
	// 	ret = adi_adrv904x_TxTestToneSet(phy->kororDevice, &toneCfg, 1);
	// 	mutex_unlock(&phy->lock);
	// 	if (ret)
	// 		return adrv904x_dev_err(phy);
...
// static irqreturn_t adrv904x_irq_handler(int irq, void *p)
// {
	// struct iio_dev *indio_dev = p;
	// struct adrv904x_rf_phy *phy = iio_priv(indio_dev);
	// adi_adrv904x_GpIntMask_t gpIntStatus;
	// int ret;

	// ret = adi_adrv904x_GpIntStatusGet(phy->kororDevice, &gpIntStatus);
	// if (ret)
	// 	return adrv904x_dev_err(phy);

	// dev_warn(&phy->spi->dev, "GPIO Interrupt Status 0x%llX 0x%llX\n",
		...
		struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		if (phy->kororDevice->devStateInfo.linkSharingEnabled == 1) {
			// ToDo - link sharing enabled review sequence
		...
		struct jesd204_link *lnk)
{
	...
	if (!priv->link[lnk->link_id].is_framer) { /* DEFRAMER */
		...
		/***********************************************************/
		/**** Enable SYSREF to Talise JESD204B/JESD204C Deframer ***/
		/***********************************************************/
		ret = adi_adrv904x_DeframerSysrefCtrlSet(phy->kororDevice,
			priv->link[lnk->link_id].source_id, ADI_DISABLE);
		if (ret)
		...
		struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		...
		if ((framerStatus.status & 0x82) != 0x0A)
		...
		for(int i = 0; i < 8; i++) {
			dev_warn(&phy->spi->dev,
				...
				lnk->link_id, deframerStatus.laneStatus[i]);
		}
	};
	...
	enum jesd204_state_op_reason reason)
{
	...
	ret = adi_adrv904x_RxTxEnableSet(phy->kororDevice, 0x00, 0x00, 
		...
		ADI_ADRV904X_TX0 | ADI_ADRV904X_TX7, ADI_ADRV904X_TX0 | ADI_ADRV904X_TX7);
	if (ret)
		return adrv904x_dev_err(phy);
...
static int adrv904x_probe(struct spi_device *spi)
{
	...
	for (i = 0; i < ADI_ADRV904X_RX_GAIN_TABLE_ARR_MAX; i++) {
		...
		pr_err("TESTIGN file name is %s\n", phy->trxBinaryInfoPtr.rxGainTable[0].filePath);
	}
	...
	// ToDO - this is overwritten by API
	...
	// 		&spi->dev, spi->irq, NULL, adrv904x_irq_handler,
